// history.rs — execution history persistence
//
// Stores entries in a JSONL file inside app_data_dir() (Tauri 2).
// UUID id is generated client-side (crypto.randomUUID()), not in Rust.
// JSONL format is append-only, crash-resistant, and human-readable.
//
// Exposed commands:
...
/// A single script execution history entry.
/// The UUID v4 id is generated client-side before calling `append_history`.
...
/// Returns the path to history.jsonl inside app_data_dir().
...
/// Tauri command — appends an entry to the end of history.jsonl.
/// Creates the file if it does not exist (append-only, never rewrite).
#[tauri::command]
...
/// Tauri command — returns the `limit` most recent entries from history.jsonl.
...
pub fn get_history(app: tauri::AppHandle, limit: usize) -> Result<Vec<HistoryEntry>, String> {
    ...
    let entries: Vec<HistoryEntry> = reader
        ...
        .filter_map(|line| {
            ...
            // Corrupted lines silently ignored
            serde_json::from_str(&line).ok()
        })
        .collect();

    // Return the `limit` most recent entries in reverse order (newest first)
    let result: Vec<HistoryEntry> = entries.into_iter().rev().take(limit).collect();
    Ok(result)
}

/// Tauri command — deletes history.jsonl.
/// If the file does not exist → Ok(()) silently.
#[tauri::command]
...
mod tests {
    ...
    // ── Helpers for testing without AppHandle (pure unit tests) ──────────────
    ...
    // TC-H01: append then get → entry retrieved
    #[test]
    ...
    // TC-H02: get_history respects the limit
    #[test]
    ...
    // TC-H04: absent file → get returns [] without error
    #[test]
    ...
    // TC-H05: reversed order — most recent entries returned first
    #[test]
    ...
    // TC-H06: corrupted lines silently ignored
    #[test]
    ...
    // TC-H07: clear_history on absent file → silent Ok
    #[test]
    ...
    // TC-H08: HistoryEntry implements Clone and Debug
    #[test]
